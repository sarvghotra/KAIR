...
def main():
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
        ...
        # ort_session = ort.InferenceSession(onnx_out_file, providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
        # outputs = ort_session.run(
        #     None,
        #     {"input_imgs": to_numpy(img_lq)},
        # )
        # #output = outputs.data.squeeze().float().cpu().numpy() #.clamp_(0, 1)
        # output = outputs[0][0]
        # print(output.shape)
        # if output.ndim == 3:
        #     output = np.transpose(output[[2, 1, 0], :, :], (1, 2, 0))  # CHW-RGB to HCW-BGR
        # output = (output).round().astype(np.uint8)  # float32 to uint8
        # print(output.shape)
        ...
        with torch.no_grad():
            ...
            dynamic_axes = {
                'input_imgs': {0:'batch_size', 1: 'channel_size', 2: 'height_size', 3: 'width_size'},
                'output_imgs': {0:'batch_size', 1: 'channel_size', 2: 'height_size', 3: 'width_size'},
            }
            torch.onnx.export(
                model,
                (img_lq),
        ...
        # output = output.data.squeeze().float().cpu().numpy() #.clamp_(0, 1)
        # if output.ndim == 3:
        #     output = np.transpose(output[[2, 1, 0], :, :], (1, 2, 0))  # CHW-RGB to HCW-BGR
        # output = (output).round().astype(np.uint8)  # float32 to uint8
        # cv2.imwrite(f'{"/tmp/onnx_out"}/{imgname}_SwinIR.png', output)
...
class SwinIR(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            assert self.upscale <= 4, 'only support x4 and lesser.'
            self.conv_before_upsample = nn.Sequential(nn.Conv2d(embed_dim, num_feat, 3, 1, 1),
                                                      nn.LeakyReLU(inplace=True))
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            x = self.conv_after_body(self.forward_features(x)) + x
            x = self.upsample(x)
        elif self.upscale == 3 and self.upsampler == 'nearest+conv':
            # for real-world SR
            x = self.conv_first(x)
            x = self.conv_after_body(self.forward_features(x)) + x
            x = self.conv_before_upsample(x)
            x = self.lrelu(self.conv_up1(torch.nn.functional.interpolate(x, scale_factor=3, mode='nearest')))
